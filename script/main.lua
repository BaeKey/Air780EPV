...
sys.taskInit(
    function()
        -- 检查是否启用定时重启
        if not config.DAILY_RESTART_ENABLED then
            log.info("main", "每日定时重启功能已禁用")
            return
        end
        
        local restart_hour = config.DAILY_RESTART_HOUR or 3  -- 默认凌晨3点重启
        log.info("main", "每日定时重启已启用，重启时间:", restart_hour .. ":00")
        
        -- 等待网络就绪后进行时间同步   
        sys.waitUntil("IP_READY", 60000)
        
        -- 启动 SNTP 时间同步
        socket.sntp({"ntp.aliyun.com", "ntp.ntsc.ac.cn", "cn.ntp.org.cn"})
        log.info("main", "正在进行时间同步...")
        
        -- 订阅时间同步事件
        sys.subscribe("NTP_UPDATE", function()
            log.info("main", "时间同步成功:", os.date("%Y-%m-%d %H:%M:%S"))
            sys.publish("TIME_SYNC_OK")
        end)
        
        sys.subscribe("NTP_ERROR", function()
            log.warn("main", "时间同步失败，重试中...")
            sys.timerStart(socket.sntp, 10000, {"ntp.aliyun.com", "ntp.ntsc.ac.cn", "cn.ntp.org.cn"})
        end)
        
        -- 等待时间同步完成，最多等待5分钟
        sys.waitUntil("TIME_SYNC_OK", 300000)
        ...
        while true do
            ...
            -- 计算到下次重启时间的秒数
            local current_seconds = now.hour * 3600 + now.min * 60 + now.sec
            local target_seconds = restart_hour * 3600
            local wait_seconds = current_seconds < target_seconds and 
                                (target_seconds - current_seconds) or 
                                (86400 - current_seconds + target_seconds)
            
            log.info("main", string.format("当前时间: %02d:%02d:%02d, 距离重启 %d 小时 %d 分钟", 
                now.hour, now.min, now.sec,
                math.floor(wait_seconds / 3600), 
                math.floor((wait_seconds % 3600) / 60)))
            
            -- 等待到重启时间
            sys.wait(wait_seconds * 1000)
            
            log.info("main", "执行每日定时重启...")
            
            -- 发送重启通知
            if util_notify and config.BOOT_NOTIFY then
                util_notify.add("设备即将执行每日定时重启 (时间: " .. os.date("%H:%M:%S") .. ")")
                sys.wait(5000)
            end
